...
/* Modified for Angband v2.8.1 by j.c.w.price@shef.ac.uk (James Denholm-Price) */

#ifdef USE_EMX

/*
 * === Instructions for using Angband 2.8.1 with OS/2 ===
 *
 * The patches (file "main-emx.c") to compile Angband 2.8.1 under OS/2
 * were written mainly by ekraemer@pluto.camelot.de (Ekkehard Kraemer).
 *
 * TO COMPILE:
 *
 ...
 *                      v5      Added map window
 *
 *   1.04.97 JCWP    2.8.1    Added Ben's new multiple windows --
 *                                just shows how good EKs stuff was :)
 */
#include <signal.h>
#include <stdio.h>
#include <stdlib.h>
...
/* Screen array -- pretty pointless? */
static termPipe angband_screen[MAX_TERM_DATA];
   ...
   USHORT i;
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		cptr name = angband_term_name[i];
      	initPipeTerm(&angband_screen[i], name, &angband_term[i]);
	}

	/* Initialize main window */
	t = (term*)(&angband_screen[0]);
...
int main(int argc, char **argv)
{
	...
	if (argc!=2 && argc!=3)
	{
		printf("Usage: %s choice|recall|mirror|term[4-7] [number of lines]\n"
